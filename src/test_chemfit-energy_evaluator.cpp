...
This script evaluates energy and force for a given configuration, based on an un-formatted parameter file. Can be used to verify parameter file is correct

This is a horribly written script and should really be integrated into the test_chemfit utility...

./a.out <.xyzf file> <number of frames> <Oe> <Or> <parameter file>
...
void params::read_params(string paramfile)
{
    ...
    for (int i=0; i<nparams; i++)
        cout << "# " << O2BD[i] << "    " << O2BE[i] << "   " << coeff[i] << endl;
...
void calc_2b(params & parameters, frame & config)
{
	xyz    rvec; 
	double rlen;
    ...
    {    
        for (int j=i+1; j<config.natoms; j++)
        {
            ...
            parameters.universalize_dist(parameters.vdw[config.elemtype[i]], parameters.vdw[config.elemtype[j]],  morse_lambda, rcin, rout);
            ...
            if ( (rlen > rcin) && (rlen < rout) ) // No special handling for r < rcin, since not allowed. Should be included later
            {
                ...
                sigmaij = parameters.transform_eion(parameters.eion[config.elemtype[i]], parameters.eion[config.elemtype[j]]);
                ...
                for (int p=0; p<parameters.coeff.size(); p++)
                {
                    ...
                    tmp_doub = -1 * Te[Oe+1] * ( fs*Trd[Or+1] + fsderiv*Tr[Or+1]);   
                    
                    tmp_ener += fs * coeff * Te[Oe+1] * Tr[Or+1];

                    // Finally, account for the x, y, and z unit vectors
                            
                    deriv = coeff * tmp_doub * rvec.x / rlen;
                    config.force[i].x += deriv;    // fx on atom i
                    config.force[j].x -= deriv;    // fx on atom j
                            
                    deriv = coeff * tmp_doub * rvec.y / rlen; 
                    config.force[i].y += deriv;
                    config.force[j].y -= deriv;
                            
                    deriv = coeff * tmp_doub * rvec.z / rlen;
                    config.force[i].z += deriv;
                    config.force[j].z -= deriv;
    ...
    for (int i=0; i<config.natoms; i++)
    {
        cout << "Force: " << config.force[i].x << endl;
        cout << "Force: " << config.force[i].y << endl;
        cout << "Force: " << config.force[i].z << endl;
    }
    
    cout << "Energy: " << tmp_ener << endl;
...
{
    ifstream infile;
    string trajfile = argv[1]; //"test-Ar+He_50F.xyzf"
    infile.open(trajfile);
    int  nframes    = stoi(argv[2]); // 50;

    params parameters(stoi(argv[3]) , stoi(argv[4])); // Arguments: eion order, rij order

    string paramfile = argv[5]; // "params.txt"

    parameters.read_params(paramfile);
    ...
    for(int i=0; i<nframes; i++)
    {
        ...
        frame config;
        config.read_frame(infile, parameters);
        
        cout << "#   ...reading frame complete" << endl; 
        
        calc_2b(parameters, config);
